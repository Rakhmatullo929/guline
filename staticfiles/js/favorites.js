...
class FavoritesManager {
    ...
    updateAllFavoriteButtons() {
        const buttons = document.querySelectorAll('.favorite-btn, .favorite-btn-left, .favorite-btn-top');
    ...
    updateFavoritesCounter() {
        ...
        console.log('Updating favorites counter:', count);
        
        counters.forEach(counter => {
            counter.textContent = count;
            counter.style.display = count > 0 ? 'inline' : 'none';
            console.log('Updated counter element:', counter);
        });
        ...
        if (favoritesLink) {
            ...
            console.log('Updated favorites link visibility:', favoritesLink.style.display);
        } else {
    ...
    /**
     * Обновление скрытого поля с данными избранного
     */
    updateFavoritesData() {
        ...
        if (favoritesData) {
            ...
            console.log('Updated favorites data field:', data);
        } else {
            console.log('Favorites data field not found');
        }
    }
     ...
     */
    bindEvents() {
        // Обработка HTMX событий для кнопок избранного
        document.addEventListener('htmx:beforeRequest', (e) => {
            if (e.target.matches('.favorite-btn, .favorite-btn-left, .favorite-btn-top')) {
                console.log('HTMX beforeRequest for favorite button');
                const productId = this.getProductIdFromButton(e.target);
                console.log('Product ID from button:', productId);
                if (productId) {
                    this.toggleFavorite(productId);
                    this.updateFavoritesData();
                    console.log('Updated favorites data:', this.favorites);
                }
            }
        });

        // Обновляем кнопки после HTMX ответа
        document.addEventListener('htmx:afterSwap', (e) => {
            if (e.target.matches('.favorite-btn, .favorite-btn-left, .favorite-btn-top')) {
                const productId = this.getProductIdFromButton(e.target);
                if (productId) {
                    this.updateFavoriteButton(e.target, productId);
                }
            }
        });
        ...
        // Обновляем данные перед каждым HTMX запросом
        document.addEventListener('htmx:beforeRequest', (e) => {
            this.updateFavoritesData();
        });
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded, initializing favorites manager...');
    window.favoritesManager = new FavoritesManager();
    
    // Принудительное обновление счетчика через небольшую задержку
    setTimeout(() => {
        if (window.favoritesManager) {
            console.log('Force updating favorites counter...');
            window.favoritesManager.updateFavoritesCounter();
        }
    }, 100);
});
